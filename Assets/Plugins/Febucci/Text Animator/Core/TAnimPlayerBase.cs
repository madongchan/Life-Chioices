...
namespace Febucci.UI.Core
{
    ...
    [System.Serializable]
    public class LineEvent : UnityEvent<float> { }
    /// <summary>
    ...
    public abstract class TAnimPlayerBase : MonoBehaviour
    {
        ...
        //[SerializeField, Tooltip("True = resets the typewriter speed every time a new text is set/shown")] bool resetTypingSpeedAtStartup = false;
        ...
        ///<summary>
        /// Custom Event added by woohyn
        /// Called once when line breaks
        /// It is only invoked when the typewriter is enabled.
        /// </summary>
        public LineEvent onLineBreak;
        ...
        private IEnumerator ShowRemainingCharacters()
        {
            ...
            if (!textAnimator.allLettersShown)
            {
				...
				while (!textAnimator.allLettersShown)
                {
                    ...
                    bool DetectLineBreak() {
                        return latestLine != textAnimator.latestCharacterShown.lineNumber;
                    }
                    //triggers event when line breaks
                    
                    

                    if (DetectLineBreak()) {
                        latestLine = textAnimator.latestCharacterShown.lineNumber;
                        onLineBreak?.Invoke(textAnimator.textInfo.lineInfo[latestLine].lineHeight);
                    }
